...
category: "인지심리학"      # 학습과학 5대 영역 권장. 또는 ["인지심리학","교육심리학"]
...
| `grade` | 대상 학년(선택). 예: `"초4"`, `"중등전체"`. 필터에 사용. |
...
블로그·가이드·툴킷 글에 `category`를 아래 값으로 지정하면 **주제별** 페이지(`/domains`), 홈의 「5가지 영역」, 목록 필터에서 동일한 순서로 노출됩니다.

| 카테고리 값 | 설명 |
|------------|------|
| `인지심리학` | 주의, 기억, 인지부하 등 인지 과정 |
| `신경과학(뇌과학)` | 뇌 구조·기능과 학습·발달 |
