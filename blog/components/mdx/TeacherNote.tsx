...
export function TeacherNote({
  ...
  return (
    <aside
      ...
      aria-label="교사 노트"
    >
      <p className="text-xs font-semibold uppercase tracking-wider text-amber-600 dark:text-amber-400">교사 노트</p>
      {context && (
        <p className="mt-2 text-sm font-medium text-slate-500 dark:text-slate-400">
          <span className="text-slate-900 dark:text-slate-100">맥락: </span>
          {context}
        </p>
      )}
      ...
      {interpretation && (
        <p className="mt-3 text-sm italic text-slate-500 dark:text-slate-400">
          <span className="font-medium not-italic text-slate-900 dark:text-slate-100">해석: </span>
          {interpretation}
        </p>
      )}
