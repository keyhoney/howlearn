...
$\angle \rm BAC$와 $\angle \rm BDA$는 삼각형 $\rm ABD$의 두 각이에요. 크기가 같다는 뜻이니까, 그 각들 **맞은편 변의 길이** 관계를 떠올려 보세요.



## 힌트

$\overline{\rm AB} = \overline{\rm BD}$가 되면 삼각형 $\rm ABD$는 **이등변삼각형**이에요. 꼭짓점 $\rm B$에서 밑변 $\overline{\rm AD}$에 내린 수선은 밑변을 이등분한다는 성질로 $\overline{\rm AD}$의 길이를 구할 수 있어요. (또는 코사인 법칙을 써도 돼요.)



## 힌트

$\overline{\rm AC} = 5$이고 $\overline{\rm AD}$를 알면 $\overline{\rm DC}$가 정해져요. $\overline{\rm BD}$와 $\overline{\rm DC}$가 같다면 삼각형 $\rm BCD$도 이등변이에요. $\rm D$에서 $\overline{\rm BC}$에 내린 수선의 발 $\rm H'$는 어디쯤일까요?



## 힌트

$\angle \rm BED = \angle \rm BAC$이고 $\cos(\angle \rm BAC) = \frac{1}{8}$이니까 $\sin(\angle \rm BAC)$도 구할 수 있어요. 점 $D$에서 $\overline{\rm BC}$에 수직인 직각삼각형을 잡고, $\overline{\rm DE}$와 수선의 길이 $\overline{\rm DH'}$를 각 $\angle \rm BED$와 연결해 보세요. (그림에서 $\angle \rm H'ED$와 $\angle \rm BED$가 맞닿는 경우를 생각하면 돼요.)
