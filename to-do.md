...
- [ ] **Build configuration**
  - **Root directory:** `blog`
  - **Framework preset:** **`Next.js`(next-on-pages) 사용 안 함** — OpenNext 경로 사용.
  - **Build command:** `npm run cf:build` (`npx opennextjs-cloudflare build` 는 CI에서 *could not determine executable to run* 으로 실패할 수 있음)
  - **Build output directory:** 대시보드·[Workers CI/CD](https://developers.cloudflare.com/workers/ci-cd/)·[OpenNext 문서](https://opennext.js.org/cloudflare/get-started) 최신 안내에 따름(`.vercel/output/static` 아님).
  - 또는 로컬/CI에서 **`npm run deploy`** 로 Wrangler 배포만 사용.

### 2.2 OpenNext + Wrangler (적용됨)

레포에 이미 반영됨: `wrangler.jsonc`, `open-next.config.ts`, `preview` / `deploy` 스크립트, `next.config.ts`의 `initOpenNextCloudflareForDev()`, `public/_headers`.

- [ ] 공식 가이드: [OpenNext — Cloudflare — Get Started](https://opennext.js.org/cloudflare/get-started)
- [ ] **Wrangler 로그인:** `npx wrangler login` (마이그레이션 시 OAuth 타임아웃 났다면 재시도)
- [ ] (선택) [R2 캐시](https://opennext.js.org/cloudflare/caching) — 마이그레이션에서 캐시 자동 설정이 실패했을 수 있음. 필요 시 문서대로 `wrangler.jsonc` / `open-next.config.ts` 보강
- [ ] **Windows**에서 `opennextjs-cloudflare build` 가 불안정하면 **WSL 또는 CI(Linux)** 에서 빌드
- [ ] `.open-next` 는 빌드 산출물 — Git에 올리지 않음

> `npm run build` 는 **`generate-rss` 후 `next build`** 이며, OpenNext 빌드가 이 스크립트를 호출하므로 RSS 생성이 포함됩니다.
