      ...
      const summary =
        typeof parsed.data.summary === 'string' && parsed.data.summary.trim()
          ...
          : typeof parsed.data.description === 'string'
            ...
            : '';
      const categories =
        ensureStringArray(parsed.data.categories).length > 0
          ? ensureStringArray(parsed.data.categories)
          : ensureStringArray(parsed.data.category);
      const publishedAt = toIsoDate(parsed.data.publishedAt) ?? toIsoDate(parsed.data.datePublished);
